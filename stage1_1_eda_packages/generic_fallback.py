"""
Generic fallback section detection for any PDF that has no dedicated
sourceN_<slug>.py module, so dropping a new file into the Drive folder no
longer stops stage1_1 with NotImplementedError.
...
2. ``generic_font_headings`` -- no usable outline: find lines set noticeably
                                 ...
                                 page's font info, so the result is cached in
                                 stage1_eda_output/<source_key>/_cache_headings.json.
...
header/footer stripping, no callout-box tags). If a book matters, write a
dedicated module (see this package's __init__.py) -- it takes precedence.
"""
import json
import re
...
def _font_heading_sections(pdf_path: Path, source_key: str) -> list[dict]:
    cache_dir = pdf_path.parents[2] / "stage1_eda_output" / source_key
    cache = cache_dir / "_cache_headings.json"
    fingerprint = [pdf_path.name, pdf_path.stat().st_size]
    data = json.loads(cache.read_text(encoding="utf-8")) if cache.exists() else None
    if data and data.get("pdf") == fingerprint:
        n_pages, headings = data["n_pages"], [tuple(h) for h in data["headings"]]
    else:
        n_pages, headings = _scan_font_headings(pdf_path)
        cache_dir.mkdir(parents=True, exist_ok=True)
        cache.write_text(
            json.dumps({"pdf": fingerprint, "n_pages": n_pages, "headings": headings}, ensure_ascii=False),
            encoding="utf-8",
        )
...
def extract_sections_with_strategy(source_key: str, pdf_path: Path, pages: list[str]) -> tuple[list[dict], str]:
    ...
    print(f"[{source_key}] generic fallback ({strategy}): {len(sections)} section(s) -- "
          "consider adding a dedicated sourceN_<slug>.py module for better boundaries")
