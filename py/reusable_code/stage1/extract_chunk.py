"""Stage 1.1 -- extract text from the source PDFs and write hierarchical chunks.
    ...
    -> detect sections (one module per book in eda_packages/) -> write parent/child chunk JSON files
...
def build_sources(cfg: Config, *, drive_files: Optional[List[dict]] = None) -> Dict[str, dict]:
    ...
    """
    from eda_packages import KNOWN_SOURCE_EDA_META, SKIPPED_FILENAMES
...
def download_if_missing(cfg: Config, sources: Dict[str, dict], source_key: str) -> Path:
    """Ensure ./eda_input/<source_key>/<filename> exists (gdown from Drive if not). Safe to re-run."""
    import gdown
...
def write_source_manifests(cfg: Config, sources: Dict[str, dict], fetch_info: Dict[str, dict]) -> None:
    """One ``rag11_data_sources``-ready row per source in eda_output/sources/source_row-N.json
...
def assemble_sections(cfg: Config, sources: Dict[str, dict], downloaded: Dict[str, Path],
    ...
    """Section detection per book, routed by FILENAME to its module in eda_packages/. A file with no
    ...
    from eda_packages import process_source
    sections_by_source = {}
