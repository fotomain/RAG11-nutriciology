...
class Paths:
    ...
    def input_root(self) -> Path:
        return self.root / "eda_input"

    @property
    def output_root(self) -> Path:
        return self.root / "eda_output"
...
def get_paths(root=None) -> Paths:
    """Project paths; ``root`` defaults to ``py/`` (this package's own parent directory) rather than the
    current directory, so eda_input/eda_output resolve correctly no matter which directory a notebook's
    kernel (or a .command script) happens to be running from."""
    return Paths(Path(root).resolve() if root is not None else Path(__file__).resolve().parents[2])
...
def discover_source_keys(paths: Paths) -> List[str]:
    """Source keys that have a manifest row in eda_output/sources/ AND a chunk folder, sorted numerically.
