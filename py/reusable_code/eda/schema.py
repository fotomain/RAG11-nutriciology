"""JSON schema + prompt shared by every OCR provider (providers/google.py, providers/aws.py).

Kept separate from recognize.py so both provider modules can import it without importing each
other or the orchestrator (which imports them).
"""
