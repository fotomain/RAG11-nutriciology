...
_cache_yoga_layout.json (common.cached_json, keyed by PDF name+size).
...
def _compute_layout(pdf_path: Path) -> dict:
    ...
    for word, numeral, expected in PADAS:
        ...
        if anchor is None:
            raise RuntimeError(f"yoga_sutra_angot: could not find the '{word}pæda' heading; layout changed?")
    ...
    if appendix is None or notices is None:
        raise RuntimeError("yoga_sutra_angot: could not find the 'Notices' appendix; layout changed?")
...
def _layout(pdf_path: Path, source_key: str) -> dict:
    return cached_json(pdf_path, source_key, "_cache_yoga_layout.json", lambda: _compute_layout(Path(pdf_path)))
