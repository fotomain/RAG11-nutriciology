"""LRM stage 3.1b engine: recognised source-language pages -> other languages, layout preserved
(adapted from YS1's ys_translate.py).

Per target language and page: translate every block with Gemini (one call per page), render a
one-page PDF that reproduces the source layout, read exact word boxes back from that PDF's text
layer, render the PNG. Block ids and chunk numbers are identical in every language.

Thin CLI wrapper: py/lrm/eda1_extract/translate.py (parses args, calls translate_source() below).
Output per language: <stage>/output/<lang>/<source_key>/{json,pages,pdf,cache}/
"""
...
from . import recognize as fr  # paths, DPI, fold(), env loading, set_stage_dir()/configure()
from .languages import LANGS

TERMS_FILENAME = "terms.txt"  # optional, in <stage>/input/: terms kept untranslated, one per line
...
def load_terms() -> str:
    terms_file = fr.STAGE / "input" / TERMS_FILENAME
...
def _split_at_gaps(words: list[dict], n: int) -> list[list[dict]]:
    """Group reading-ordered fragments into n contiguous words, cutting at the n-1 widest gaps.

    Shaped Devanagari comes back as touching/overlapping fragments (matras overlap their consonant), while real
    word spaces leave a positive gap; a line break counts as the widest gap of all. Matras are drawn after
    their neighbours in the content stream, so fragments are first put in x order within each line."""
    lines: list[list[dict]] = []
    for w in words:
        b = w["bbox"]
        if lines and abs((lines[-1][0]["bbox"]["y"] + lines[-1][0]["bbox"]["h"] / 2) - (b["y"] + b["h"] / 2)) <= b["h"] / 2:
            lines[-1].append(w)
        else:
            lines.append([w])
    words = [w for line in lines for w in sorted(line, key=lambda w: w["bbox"]["x"])]
    gaps, right = [], words[0]["bbox"]["x"] + words[0]["bbox"]["w"]
    for k in range(1, len(words)):
        a, b = words[k - 1]["bbox"], words[k]["bbox"]
        if abs((a["y"] + a["h"] / 2) - (b["y"] + b["h"] / 2)) > max(a["h"], b["h"]) / 2:
            gaps.append((1.0, k)); right = b["x"] + b["w"]
            continue
        gaps.append((b["x"] - right, k))
        right = max(right, b["x"] + b["w"])
    cuts = sorted(k for _, k in sorted(gaps, key=lambda g: -g[0])[:n - 1])
    return [words[i:j] for i, j in zip([0] + cuts, cuts + [len(words)])]


def restore_script(words: list[dict], intended: str) -> list[dict]:
    """The rendered PDF's text layer does not round-trip shaped Devanagari (conjuncts come back as
    Latin-Extended junk, split into several "words"). Align the extracted words with the text that
    was actually rendered and, wherever the rendered text has Devanagari, replace the extracted
    junk with the true tokens -- keeping the extracted geometry (union box per restored token)."""
    tokens = MARKUP.sub(lambda m: m.group(1) or m.group(2), intended).split()
    if not words or not any(DEVA.search(t) for t in tokens):
        return words
    tkey = ["#deva" if DEVA.search(t) else fr.fold(t) for t in tokens]
    wkey = [fr.fold(w["text"]) for w in words]
    out: list[dict] = []
    for op, i1, i2, j1, j2 in SequenceMatcher(None, tkey, wkey, autojunk=False).get_opcodes():
        seg_t, seg_w = tokens[i1:i2], words[j1:j2]
        if op != "replace" or not any(DEVA.search(t) for t in seg_t):
            out.extend(seg_w)
            continue
        n = min(len(seg_t), len(seg_w))
        groups = _split_at_gaps(seg_w, n)
        texts = seg_t[:n - 1] + [" ".join(seg_t[n - 1:])]
        for text, g in zip(texts, groups):
            out.append({**g[0], "text": text, "bbox": _union([w["bbox"] for w in g]),
                        "lang": "sa" if DEVA.search(text) else g[0]["lang"]})
    return out
...
def extract_page(fr_page: dict, texts: dict, layout: dict, pdf_page: fitz.Page, lang: str, model: str,
    ...
    for b in fr_page["blocks"]:
        ...
        words = restore_script(words, texts.get(b["id"], b["text"]))
        for k, w in enumerate(words):
            words[k] = {"id": f"{b['id']}_w{k + 1:03d}", **w, "char_start": pos, "char_end": pos + len(w["text"])}
            parts.append(w["text"])
            pos += len(w["text"]) + 1
        if words:
...
def translate_source(pdf: Path, source_key: str, src_lang: str, langs: list[str], *, start: int | None = None,
                      end: int | None = None, workers: int = 4, model: str | None = None, force: bool = False,
                      render_only: bool = False) -> int:
    """Translate the already-recognised `source_key` [`src_lang`] pages into every language in
    `langs` (fr.configure() is called here, so recognize.set_stage_dir() must already have run).
    Returns the number of (language, page) translations that failed.

    start defaults to START_PAGE_NUMBER, end to start + MAX_NUMBER_OF_PAGES_TO_USE - 1 -- see
    recognize.start_page()/resolve_end()."""
    configured_langs = languages()
    for l in langs:
        if l == src_lang or l not in configured_langs:
            sys.exit(f"Unknown target language {l!r}; configured: {[k for k in configured_langs if k != src_lang]}")
    fr.configure(src_lang, source_key)
    start = start or fr.start_page()
    end = fr.resolve_end(start, end)
    model = model or fr.DEFAULT_MODEL
    src_doc = fitz.open(str(pdf))
    fr_pages = {}
    for f in sorted(fr.JSON_DIR.glob("page_*.json")):
        d = json.loads(f.read_text(encoding="utf-8"))
        if start <= d["page"] <= (end or 10**9):
            fr_pages[d["page"]] = d
    if not fr_pages:
    ...
    for lang in langs:
        ...
        def job(n: int) -> tuple[int, dict]:
            ...
            res = translate_page(client, model, lang, fr_pages[n], prev_context(n), terms)
