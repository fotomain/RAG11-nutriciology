...
(607 pages) -- a different PDF conversion of the SAME book as source8
...
via iText (vs. source8's converter), with a much richer native outline: 548
entries across 4 levels (vs. source8's 176 across 4 levels, heavily
...
as source5/9's Foreword/Preface); and "Appendices" (level 1) --
...
same call source8 made for this same book's answer key.
...
SPECIAL CASE -- "Appendices": unlike source8 (whose PDF converter gave
...
Boundaries otherwise use the same level-aware rule as source7/8/11 -- a
...
into whichever kept section precedes it (the source5/9 bug).

No reliable recurring callout-box marker was found beyond plain section
headers ("OBJECTIVES", "REVIEW QUESTIONS") -- the same conclusion source8
...
FILENAME = "vdoc.pub_medical-nutrition-and-disease-a-case-based-approach.pdf"
EXPECTED_PAGES = 607
STRUCTURE = "native_outline_pattern_filtered_appendix_page_split"
...
def _split_appendices(pages: list[str], start: int, end: int) -> list[dict]:
    ...
    for i, (pno, letter, desc) in enumerate(hits):
        ...
        body = "\n\n".join(pages[pno:sec_end + 1]).strip()
        out.append({
            "title": f"Appendix {letter}: {desc}",
            "level": 2,
            "start_page": pno,
            "end_page": sec_end,
            "text": body,
...
def extract_sections(source_key: str, pdf_path: Path, pages: list[str]) -> list[dict]:
    ...
    for i, (lvl, title, start) in enumerate(full_entries):
        if lvl == 1 and title in _LEVEL1_COALESCE_TITLES:
            review_answers_range = (start, max(compute_end(i), start))
            break
    ...
    for i, (lvl, title, start) in enumerate(full_entries):
        ...
        if lvl == 1 and title in _LEVEL1_COALESCE_TITLES:
            ...
            body = "\n\n".join(pages[start_r:end_r + 1]).strip()
            sections.append({
                "title": title,
                "level": lvl,
                "start_page": start_r,
